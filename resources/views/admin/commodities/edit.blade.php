...
<head>
    <title>Ubah Data Komoditas - Admin</title>
...
<body class="min-h-screen bg-gray-100">
    ...
    <header class="bg-gradient-to-r from-gray-800 via-red-900 to-gray-800 text-white py-6 shadow-lg">
        <div class="container mx-auto px-4">
            <h1 class="text-3xl font-bold text-center">UBAH DATA KOMODITAS</h1>
    ...
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="bg-white rounded-xl shadow-lg p-6 border border-gray-200">
            <h2 class="text-2xl font-bold text-gray-800 mb-4">Ubah Data Komoditas</h2>

            <form action="{{ route('admin.commodities.update', $commodity->id) }}" method="POST" class="space-y-4">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <input type="text" name="name" required value="{{ old('name', $commodity->name) }}"
                ...
                <div>
                    ...
                    <input type="date" name="date" required value="{{ old('date', $commodity->date) }}"
                        class="w-full px-4 py-2 border-2 border-gray-300 rounded-lg focus:border-red-800 focus:ring-2 focus:ring-red-800/20 outline-none">
                    @error('date')
                        <small class="text-red-500">{{ $message }}</small>
                    @enderror
                ...
                <div>
                    ...
                    <input type="number" name="price" required step="0.01" min="0" value="{{ old('price', $commodity->price) }}"
                        class="w-full px-4 py-2 border-2 border-gray-300 rounded-lg focus:border-red-800 focus:ring-2 focus:ring-red-800/20 outline-none"
                ...
                <div>
                    ...
                    <select name="type" required
                        ...
                        <option value="perhiasan" {{ old('type', $commodity->type) == 'perhiasan' ? 'selected' : '' }}>Perhiasan</option>
                        <option value="antam" {{ old('type', $commodity->type) == 'antam' ? 'selected' : '' }}>Antam</option>
                        <option value="dolar" {{ old('type', $commodity->type) == 'dolar' ? 'selected' : '' }}>Kurs Dolar</option>
                    </select>
                    @error('type')
                        <small class="text-red-500">{{ $message }}</small>
                    @enderror
